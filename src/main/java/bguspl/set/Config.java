...
public class Config {
    ...
    /**
     * Difficulty level per computer player (easy/medium/hard).
     */
    public final String[] computerLevels;
    ...
    public Config(Logger logger, Properties properties) {
        ...
        String[] levelTokens = properties.getProperty("ComputerLevels", "easy").split(",");
        computerLevels = new String[computerPlayers];
        if (computerPlayers > 0) {
            for (int i = 0; i < computerPlayers; i++) {
                int idx = Math.min(i, levelTokens.length - 1);
                computerLevels[i] = levelTokens[idx].trim().toLowerCase();
                if (computerLevels[i].isEmpty()) computerLevels[i] = "easy";
            }
        }
    ...
    /**
     * Returns configured difficulty for a given computer player index (0-based among computer players).
     */
    public String computerLevel(int computerIndex) {
        if (computerIndex < 0 || computerIndex >= computerLevels.length) return "easy";
        return computerLevels[computerIndex];
    }
}
