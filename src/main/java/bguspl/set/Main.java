...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < players.length; i++) {
            boolean human = i < env.config.humanPlayers;
            String aiLevel = human ? null : env.config.computerLevel(i - env.config.humanPlayers);
            players[i] = new Player(env, dealer, table, i, human, aiLevel);
        }
